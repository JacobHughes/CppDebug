...
Logger::Logger()
{
	...
	{
		this->fullLog.open("LogFile_" + dateAddition + "_fullLog.txt", std::ofstream::out | std::ofstream::app);
	}

	//Create the critical log stream
	while (!criticalLog.is_open())
	{
		this->criticalLog.open("LogFile_" + dateAddition + "_criticalLog.txt", std::ofstream::out | std::ofstream::app);
	}
}
